...
yalmip('clear')
% Define variables
%vi vill göra om på formen x_k+1=ax_k+bu_k
r=1;
%Xnext(3*Nv,1)
for i=1:Nv
    z = sdpvar(Ns,1);% lethargy (inverse speed)
    Xnext(r,:)=z;
    r=r+1;
    
    t = sdpvar(Ns+1,1);% time
    Xnext(r,:)=t;
    r=r+1;
    
    dz = sdpvar(Ns-1,1);% 1st derivative of lethargy
    Xnext(r,:)=dz;
    r=r+1;
    
    ddz = sdpvar(Ns-2,1);  % 2nd derivative of lethargy
    Xnext(r,:)=ddz;
    r=r+1;
