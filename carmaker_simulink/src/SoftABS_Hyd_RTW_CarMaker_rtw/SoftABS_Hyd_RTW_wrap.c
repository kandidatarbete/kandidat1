...
#include "SoftABS_Hyd_RTW.h"
#include "SoftABS_Hyd_RTW_wrap.h"
...
extern const char SoftABS_Hyd_RTW_LibIdent[];
const char SoftABS_Hyd_RTW_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_SoftABS_Hyd_RTW *rtm)
{
...
static void
SoftABS_Hyd_RTW_DeclQuants (void *MP)
{
    rtModel_SoftABS_Hyd_RTW *rtm = (rtModel_SoftABS_Hyd_RTW *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(SoftABS_Hyd_RTW_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(SoftABS_Hyd_RTW_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'\n",
		    Modelname, SoftABS_Hyd_RTW_DictDefines[i].Name);
	}
...
/*
 * SoftABS_Hyd_RTW_SetParams() will be invoked indirectly by the generated
 * model C code each time SoftABS_Hyd_RTW_New() is called.
 */
void
SoftABS_Hyd_RTW_SetParams (rtModel_SoftABS_Hyd_RTW *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
             ...
             const char *KindKey, void *MP)
{
    ...
    ExternalInputs_SoftABS_Hyd_RTW  *in  = (ExternalInputs_SoftABS_Hyd_RTW *) rtmGetU(rtm);
...
static void *
SoftABS_Hyd_RTW_New (struct tInfos *Inf, struct tHydBrakeCU_CfgIF *CfgIF, const char *KindKey)
{
    rtModel_SoftABS_Hyd_RTW *rtm;
    ...
    MatSupp_ResetQuants(SoftABS_Hyd_RTW_DictDefines);
...
static void
SoftABS_Hyd_RTW_Delete (void *MP)
{
    rtModel_SoftABS_Hyd_RTW *rtm = (rtModel_SoftABS_Hyd_RTW *)MP;
...
static int
SoftABS_Hyd_RTW_Calc (void *MP, tHydBrakeCU_IF *IF, double dt)
{
    rtModel_SoftABS_Hyd_RTW *rtm = (rtModel_SoftABS_Hyd_RTW *)MP;
    int osCount=0;
    ExternalInputs_SoftABS_Hyd_RTW  *in  = (ExternalInputs_SoftABS_Hyd_RTW *) rtmGetU(rtm);
    ExternalOutputs_SoftABS_Hyd_RTW *out = (ExternalOutputs_SoftABS_Hyd_RTW *)rtmGetY(rtm);
...
SoftABS_Hyd_RTW_Register (void)
{
    ...
    m.HydBrakeControl.DeclQuants	= SoftABS_Hyd_RTW_DeclQuants;
    m.HydBrakeControl.New		= SoftABS_Hyd_RTW_New;
    m.HydBrakeControl.Calc		= SoftABS_Hyd_RTW_Calc;
    m.HydBrakeControl.Delete		= SoftABS_Hyd_RTW_Delete;
