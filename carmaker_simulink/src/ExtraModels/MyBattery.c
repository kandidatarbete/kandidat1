/* $Id: MyBattery.c,v 1.3 2016/03/18 14:32:58 fh Exp $ (c) IPG */
/*
******************************************************************************
...
MyModel_New (struct tInfos *Inf, struct tPTBatteryCfgIF *CfgIF, const char *KindKey, const char *Ident)
{
    ...
    /* Capacity Ah */
    mp->Capacity = fabs(iGetDblOpt (Inf, strcat(strcpy(buf, PreKey), "Capacity"), 10.0));
    if (mp->Capacity==0.0) {
	LogErrF (EC_Init, "%s: Parameter '%s' must be positive and non zero", MsgPre, key);
	goto ErrorReturn;
    }

    mp->Volt_oc0 = fabs(iGetDblOpt (Inf, strcat(strcpy(buf, PreKey), "Voltage_oc"), 12.0));

    /* Model -> CfgIF */
    CfgIF->Capacity = mp->Capacity;
    CfgIF->Voltage  = mp->Volt_oc0;

    CfgIF->SOC_init = iGetDblOpt (Inf, strcat(strcpy(buf, PreKey), "SOC"), 70.0);
    CfgIF->SOC_init = M_BOUND (0.0, 100.0, CfgIF->SOC_init);

    CfgIF->SOC_min = iGetDblOpt (Inf, strcat(strcpy(buf, PreKey), "SOC_min"), 10.0);
    CfgIF->SOC_max = iGetDblOpt (Inf, strcat(strcpy(buf, PreKey), "SOC_max"), 90.0);
