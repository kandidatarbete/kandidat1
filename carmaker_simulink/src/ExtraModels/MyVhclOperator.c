...
struct tMyModel {
	...
	double	Brake;
	double	BrakePark;
    } Absent, PowerOff, PowerAcc, PowerOn, Starting, Driving, Braking;
};
...
static void
SetOutput (struct tMyModel *mp, struct tIPGOperator_State *State, tVhclOperatorIF *IF)
{
    if (mp->GearBoxKind==GBKind_Manual) {
...
static void
SetOutputAndWait (struct tMyModel *mp, struct tIPGOperator_State *State, tVhclOperatorIF *IF)
{
    switch (mp->Phase) {
	case 0:
    ...
    const char 			*KindKey)
{
    ...
    struct tIPGOperator_State *State = NULL;
    char MsgPre[64];
