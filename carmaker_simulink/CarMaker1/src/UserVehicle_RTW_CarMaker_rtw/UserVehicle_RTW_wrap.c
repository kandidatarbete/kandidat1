...
#include "UserVehicle_RTW.h"
#include "UserVehicle_RTW_wrap.h"
...
extern const char UserVehicle_RTW_LibIdent[];
const char UserVehicle_RTW_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_UserVehicle_RTW *rtm)
{
...
static void
UserVehicle_RTW_DeclQuants (void *MP)
{
    rtModel_UserVehicle_RTW *rtm = (rtModel_UserVehicle_RTW *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(UserVehicle_RTW_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(UserVehicle_RTW_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'\n",
		    Modelname, UserVehicle_RTW_DictDefines[i].Name);
	}
...
/*
 * UserVehicle_RTW_SetParams() will be invoked indirectly by the generated
 * model C code each time UserVehicle_RTW_New() is called.
 */
void
UserVehicle_RTW_SetParams (rtModel_UserVehicle_RTW *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
{
    rtModel_UserVehicle_RTW *rtm = (rtModel_UserVehicle_RTW *)MP;
    ExternalInputs_UserVehicle_RTW  *in  = (ExternalInputs_UserVehicle_RTW *) rtmGetU(rtm);
...
static void *
UserVehicle_RTW_New (struct tInfos *Inf)
{
    rtModel_UserVehicle_RTW *rtm;
    double rtmSampleTime;

    /*Log("%s_New()\n", Modelname);*/

    MatSupp_ResetQuants(UserVehicle_RTW_DictDefines);
...
static void
UserVehicle_RTW_Delete (void *MP)
{
    rtModel_UserVehicle_RTW *rtm = (rtModel_UserVehicle_RTW *)MP;
...
static int
UserVehicle_RTW_Calc (void *MP, double dt)
{
    rtModel_UserVehicle_RTW *rtm = (rtModel_UserVehicle_RTW *)MP;
    int osCount=0;
    //ExternalInputs_UserVehicle_RTW  *in  = (ExternalInputs_UserVehicle_RTW *) rtmGetU(rtm);
    //ExternalOutputs_UserVehicle_RTW *out = (ExternalOutputs_UserVehicle_RTW *)rtmGetY(rtm);
...
VehicleModel_New (struct tInfos *Inf)
{
    ...
    if ((MP = UserVehicle_RTW_New(Inf)) == NULL)
	return NULL;
...
static void
CM2VehicleModel (ExternalInputs_UserVehicle_RTW *in)
{
...
static void
VehicleModel2CM (const ExternalOutputs_UserVehicle_RTW *out)
{
...
VehicleModel_Calc (void *MP, double dt)
{
    ...
    ret = UserVehicle_RTW_Calc(MP, dt);
...
UserVehicle_RTW_Register (void)
{
    ...
    m.Vehicle.DeclQuants	= UserVehicle_RTW_DeclQuants;
    m.Vehicle.New		= VehicleModel_New;
    m.Vehicle.Calc		= VehicleModel_Calc;
    m.Vehicle.Delete		= UserVehicle_RTW_Delete;
