...
#if defined (ADTF_C_EXAMPLE)
#include <adtf.h>
static tTrafficObj MyTrafficObject;
#endif
...
#if defined (ADTF_C_EXAMPLE)
/*** Example: Hook function for ADTF data exchange	***************** Begin Example	***/


/*
** User_ADTF_TxHook ()
**
** This function will be called, just before data is sent to ADTF
**
** Return:
**  0	ok
** -1	an error occurred
...
static int
User_ADTF_TxHook (char* streamName) {
    if (strcmp(streamName, "CMSend_raw")==0) {
	/* do something... */
    }
...
/*** Example: Hook function for ADTF data exchange	***************** End Example	***/
#endif
...
{
#if defined (ADTF_C_EXAMPLE)
    ADTF_TxHookFct = User_ADTF_TxHook;
    ADTF_RequestData ("TrafficFeedback_raw", &MyTrafficObject, sizeof(tTrafficObj),'r');
#endif
...
#endif
#if defined (ADTF_C_EXAMPLE)
    DDefDouble4 (NULL, "MyTrafObj.v", "m/s", &MyTrafficObject.LongVel, DVA_None);
    DDefDouble4 (NULL, "MyTrafObj.Dist", "m", &MyTrafficObject.Distance, DVA_None);
    DDefDouble4 (NULL, "MyTrafObj.sRoad", "m", &MyTrafficObject.sRoad, DVA_None);
    DDefDouble4 (NULL, "MyTrafObj.tx", "m", &MyTrafficObject.t_0[0], DVA_None);
    DDefDouble4 (NULL, "MyTrafObj.ty", "m", &MyTrafficObject.t_0[1], DVA_None);
    DDefDouble4 (NULL, "MyTrafObj.tz", "m", &MyTrafficObject.t_0[2], DVA_None);
#endif
#if defined(WITH_FLEXRAY)
    if (IO_DemoFR)
        RBS_DeclQuants();
...
# if defined (ADTF_C_EXAMPLE)
    if (Traffic.nObjs>0) {
	ADTF_RequestData ("TrafficInfo_raw", &Traffic.Objs[0], sizeof(tTrafficObj),'s');
    } else {
	ADTF_DeleteDataRequest_byName ("TrafficInfo_raw");
    }
# endif
