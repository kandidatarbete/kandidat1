/* $Id: MyEngineCU.c,v 1.3 2016/03/18 14:32:58 fh Exp $ (c) IPG */
/*
******************************************************************************
...
MyModel_New (struct tInfos *Inf, struct tPTEngineCU_CfgIF *CfgIF, const char *KindKey)
{
    ...
    mp->ISCtrl.I       = iGetDblOpt  (Inf, "PowerTrain.MyEngineCU.ISCtrl.I", 	  0.01);
...
MyModel_Calc (void *MP, struct tPTEngineCU_IF *IF, double dt)
{
	...
	if (IF->set_ISC) {
		...
		if (IF->Load < 1e-3) {
		    mp->ISCtrl.Load_I += drotv * mp->ISCtrl.I;
