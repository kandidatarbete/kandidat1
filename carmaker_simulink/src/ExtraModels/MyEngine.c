    ...
    const char		  *Ident)
{
    ...
    mp->rotvIdle   = iGetDbl(Inf, "PowerTrain.Engine.rotv_idle") * rpm2radsec;
    mp->rotvOff    = iGetDbl(Inf, "PowerTrain.Engine.rotv_off")  * rpm2radsec;

    /* Inertia */
    mp->I_out = iGetDblOpt (Inf, "PowerTrain.Engine.I", 0.1);

    n = 0;
    key = "PowerTrain.Engine.TrqFull";
    if ((fv = iGetTableFlt2(Inf, key, 2, &n)) != NULL) {
	...
	if ((mp->TrqFull = LMInit(fv, fv+n, n, ArrayNotAequi)) == NULL) {
	    LogErrF (EC_Init, "%s: Can't initialize full load look-up table '%s'",
		MsgPre, key);
	    goto ErrorReturn;
	}
	free (fv);
	...
	if ((mp->TrqDrag = LMInit(fv, fv+n, n, ArrayNotAequi)) == NULL) {
	    LogErrF (EC_Init, "%s: Can't initialize drag look-up table '%s'",
		MsgPre, key);
	    goto ErrorReturn;
	}
	free (fv);
    ...
    /* Model -> CfgIF */
    CfgIF->rotv_off  = mp->rotvOff;
    CfgIF->rotv_max  = mp->rotvMax;
    CfgIF->rotv_idle = mp->rotvIdle;
    CfgIF->TrqFull   = mp->TrqFull;
    CfgIF->TrqDrag   = mp->TrqDrag;
