...
struct tMyModel {
    /* Parameters */
    tTParamHeader Head;			/* Struct must begin with the header */

    struct {
	char		*path;		/* Path of RTTire file name */
	unsigned int	ModTime;	/* Modification time of RTTire file name */
    } TireFile;

    struct tModelClassDescr_Tire Wrapped;	/* wrapped model: functions   *
						 * (New/Calc/Delete/...)      */
    ...
    int			Twin)
{
    ...
    tModelClassDescr *md;
    char        *key, MsgPre[64], *path;
    char 	ModelKind[128] = 	"";
    int		VersionId = 0;
    char	*item;
    const char *FName;
    struct tInfos *Inf_RTTire = NULL;
    struct tErrorMsg *perrors;
    ...
    Inf_RTTire = InfoNew();
    if (iRead(&perrors, Inf_RTTire, path) == -1) {
	LogErrF(EC_Init, "%s: Can't read RTTire file '%s'", MsgPre, path);
	goto ErrorReturn;
    }
