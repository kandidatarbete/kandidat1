...
MyModel_New (tInfos *Inf, struct tPTGearBoxCfgIF *CfgIF, const char *KindKey, const char *Ident)
{
    ...
    /* Get parameters for gears */
    n = 0;
    iGetTable(Inf, "PowerTrain.GearBox.iForwardGears", dvec, VEHICLE_NGEARS, 1, &n);
    if (n < 0 || n > D_GEARS) {
	LogErrF (EC_Init, "%s: Wrong number of forward gears, got %d (0 .. %d)",
	    MsgPre, n, D_GEARS);
	goto ErrorReturn;
    }
    mp->nFGears = n;
    ...
    n = 0;
    iGetTable(Inf, "PowerTrain.GearBox.iBackwardGears", dvec, VEHICLE_NGEARS, 1, &n);
    if (n < 0 || n > D_GEARS) {
	LogErrF (EC_Init, "%s: Wrong number of backward gears, got %d (0 .. %d)",
	    MsgPre, n, D_GEARS);
	goto ErrorReturn;
    }
    mp->nBGears = n;
    ...
    /* Model to Vehicle */
    CfgIF->nFGears = mp->nFGears;
    for (i=0; i <= mp->nFGears; i++)
	CfgIF->iFGear[i] = mp->iF[i];

    CfgIF->nBGears = mp->nBGears;
    for (i=0; i <= mp->nBGears; i++)
	CfgIF->iBGear[i] = mp->iB[i];

    /* No automatic gearbox */
    CfgIF->GBKind = GBKind_Manual;

    CfgIF->ClKind = ClKind_Closed;
